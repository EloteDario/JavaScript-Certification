...
const updateResultsList = () => {
  ...
  units.forEach(unit => {
    if (units !== unitToConvert.value) {
      const number = processIngredient(ingredientQuantity.value, unitToConvert.value, unit, numberOfServings.value)

      resultList.innerHTML += `<li>${ingredientName.value}: ${number} ${unit}</li>`
    }
  })
